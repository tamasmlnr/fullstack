...
const Menu = ({ anecdotes, addNew, anecdoteById, message }) => {
  const padding = {
    paddingRight: 5
  }
  ...
  return (
    <Router>
      <div>
        ...
        <Route path="/create" render={() => !message ? <CreateNew addNew={addNew} /> : <Redirect to="/" /> } />
...
const Anecdote = ({ anecdote }) => {
  return <div><h1>{anecdote.content}</h1>
...
const App = () => {
  ...
  return (
    <div>
      ...
      {notification}
      <Menu anecdotes={anecdotes} addNew={addNew} anecdoteById={anecdoteById} message = {notification} />
      <Footer />
    </div>
  )
}
